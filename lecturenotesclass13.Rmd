...
```{r data_manhattan}
manhattan <- read_csv("~/R/lecture-note-class-13-EtanZeller/manhattan.csv")
```
...
- Population: one bedroom apartments in Manhattan
...
```{r practice_1}
boot_medians <- manhattan %>% 
  specify(response = rent) %>% 
  generate(reps = 10000, type = "bootstrap") %>%
  calculate(stat = "median")
get_ci(boot_medians, level = 0.95)
```
